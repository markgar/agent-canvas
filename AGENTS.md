...
- Keep each workspace modular. Do not introduce infrastructure, persistence, or
  a frontend framework without a concrete need. Add a package only for a real
  second consumer or an independent runtime/release boundary.
- Contracts in `apps/canvas/src/contracts/` must remain usable without Node.js
  or browser globals. Infer wire types from runtime schemas.
